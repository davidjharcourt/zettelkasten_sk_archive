...
(function(module) {
    ...
    P.when('A').execute(function(A) {
        ...
        var Modal = function() {
            ...
            this.show = function(repaint, config) {
                if (weblabResp === "T1" && (/Android/.test(navigator.userAgent))
    ...
    */
    P.when('A', 'wlp-injectable-modal', 'usp-wlp-utility').register(
        ...
        function(A, modal, utility) {
            ...
            var preFetchIdChain = {};
            var activePreFetchIdState = {};
            var _activePrefetchId = "";
            ...
            var _loadPopup = function(input, preFetchId) {
                ...
                if (preFetchId) {
                    _activePrefetchId = preFetchIdChain[preFetchId] || preFetchId;
                    activePreFetchIdState[_activePrefetchId] = activePreFetchIdState[_activePrefetchId] || {};
                    activePreFetchIdState[_activePrefetchId].executedActionsList = activePreFetchIdState[_activePrefetchId].executedActionsList || [];
                }

                if (!isAuthPortal) {
                    modal.show(undefined, utility.PreFetchService().getConfig(_activePrefetchId).request);
                } else {
                ...
                if (preFetchId) {
                    ...
                    //In case of multiple instances of injectable on a page force fetch the content/dont take it from cache
                    if (Object.keys(preFetchIdChain).length > 1) {
                        forceFetch = true;
                        showPopUpSpinner({
                            msg: 'Processing',
                            width: '100px',
                    ...
                    utility.PreFetchService().get(preFetchId, function(success, result, togglePreFetch) {
                        ...
                        P.when('wlp-injectable-execute-action').execute(function(action) {
                            var clientConfig = utility.PreFetchService().getConfig(preFetchId) || {};
                            action.execute(clientConfig.request);
                        });

                    }, undefined, forceFetch);
            ...
            var _declineOfferAction = function() {
                var clientReqMetaData = utility.PreFetchService().getConfig(_activePrefetchId);
                if (clientReqMetaData && clientReqMetaData.request && clientReqMetaData.request.cancelRedirectURL && 
                    clientReqMetaData.request.cancelRedirectURL.trim().length !== 0) {
                    window.location.href = clientReqMetaData.request.cancelRedirectURL;
                } else {
                    _closeModal();
                }
                return true;
            ...
            var redirectOnSuccess = function() {
                var clientReqMetaData = utility.PreFetchService().getConfig(_activePrefetchId);
                var redirectOnSuccessUrl = clientReqMetaData && clientReqMetaData.request.redirectURL;
                window.location.href = redirectOnSuccessUrl || '/gp/prime';
                return true;
            };
        ...
        (function(WlpInjectable, _, $) {
            ...
            WlpInjectable.PreFetchService = function() {
                var Service = function() {
                    ...
                    /**
                    * @public
                    * returns the registered client config
                    * @param {String} prefecthId
                    * @return {Object} client config
                    */
                    this.getConfig = function(id) {
                        return _cache[id].getConfig();
                    };
                     ...
                     */
                    this.get = function(id, onComplete, config, forceFetch) {
                        ...
                        if (_cache[id]) {
                            _cache[id].getData(onComplete, forceFetch);
                            return id;
                        }
            ...
            (function(WlpInjectable, $, _) {
                ...
                var Widget = function(provider, jQEle) {
                    ...
                    this.register = function() {
                        ...
                        var clientQueryParams = _.getUrlParams()["wlpLocation"];
                        if(clientQueryParams && clientQueryParams.indexOf(config.clientId) !== -1 
                            && clientQueryParams.indexOf(config.ingressId) !== -1){
                            signInRedirect && ping();
                        }
                        // call to register the click event of the button
                        jQEle.bind('click', ping);
                    };
                ...
                _.defer(function() {
                    ...
                    $.each(ele, function(i, jqE) {
                        //Force disable prefetch if mulitiple injectables are present on the page
                        if (ele.length > 1) {
                            $(jqE).attr('data-disable-prefetch',"true");
                        }
                        (new Widget(provider(), $(jqE))).register();
                    });
